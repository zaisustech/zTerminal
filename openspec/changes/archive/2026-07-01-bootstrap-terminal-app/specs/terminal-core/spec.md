...
The terminal SHALL support a selectable ANSI color scheme, defaulting to a vibrant
"Liquid Glass" 16-color palette (with a "System" option that uses the emulator's
defaults). The cursor SHALL use the theme accent color. The choice SHALL persist
and apply to the terminal's rendered colors.
...
- **WHEN** the user selects the System scheme in Settings
- **THEN** the terminal uses the emulator's default ANSI colors
