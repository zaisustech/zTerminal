...
open class Terminal {
    ...
    // MARK: zTerminal fork additions (see ThirdParty/SwiftTerm/MODIFICATIONS.md)

    /// Total number of lines held in the current buffer — scrollback plus the
    /// on-screen rows — indexed `0..<bufferLineCount` in the SAME coordinate space
    /// as `getTopVisibleRow()` (i.e. `buffer.yDisp`). Added so the app can search
    /// the entire scrollback; the upstream API only exposes the visible window.
    public var bufferLineCount: Int {
        buffer.lines.count
    }

    /// Returns a line by its absolute index into the current buffer,
    /// `0..<bufferLineCount`. Index 0 is the oldest scrollback line; a line is
    /// on-screen when `getTopVisibleRow() <= index < getTopVisibleRow() + rows`.
    /// Returns nil when out of range. (zTerminal fork addition.)
    public func bufferLine(atIndex index: Int) -> BufferLine? {
