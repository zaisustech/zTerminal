...
open class TerminalView: NSView, NSTextInputClient, NSUserInterfaceValidations, TerminalDelegate {
    ...
    /// Size of a single character cell in points. The app uses this to map buffer
    /// (row, column) positions to view rectangles for the search highlight overlay
    /// and minimap. Returns `.zero` before the view has been laid out.
    /// (zTerminal fork addition — see ThirdParty/SwiftTerm/MODIFICATIONS.md.)
    public var cellSize: CGSize {
        guard cellDimension != nil else { return .zero }
